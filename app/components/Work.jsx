  ...
  return (
    ...
    id="work" className="w-full px-[12%] py-10 scroll-mt-20">
      ...
      className="text-center max-w-2xl mx-auto mt-5 mb-12 font-Ovo">
        Welcome to my portfolio! Explore my projects showcasing my expertise in full-stack development, application security, and DevSecOps, using technologies like Spring Boot, Java, React.js, and database management with Oracle and PostgreSQL.
      </motion.p>
      ...
      className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 my-10 dark:text-black">
        {workData.map((project, index) => (
          ...
          transition={{duration: 0.3}}
            key={index}
            className="relative group cursor-pointer rounded-lg overflow-hidden bg-white shadow-md aspect-[16/9]" // Ratio plus équilibré
          >
            {/* Image qui remplit totalement son cadre */}
            <div className="relative w-full h-full">
              <Image
                src={project.bgImage}
                alt={project.title}
                layout="fill"
                objectFit="cover" // Remplit toute la carte sans espaces blancs
                className="rounded-lg"
              />
            </div>

            {/* Conteneur du texte et de l'icône */}
